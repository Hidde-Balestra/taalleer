...
- **Woorden hardop laten uitspreken** — een uitspraakknopje (tekst-naar-spraak) bij woorden in de woordenlijst, oefening, vervoegingstoets en grammaticavoorbeelden. Gebruikt de tekst-naar-spraak-engine van het besturingssysteem (`android.speech.tts.TextToSpeech`), dus geen Google-account of Google Play Services nodig — op toestellen zonder enige geïnstalleerde engine (bv. GrapheneOS) toont de app een duidelijke melding met de suggestie om een open-source engine zoals [eSpeak NG](https://f-droid.org/packages/com.reecedunn.espeak/) via F-Droid te installeren. De fonetische uitspraak staat sowieso ook altijd als tekst bij elk woord en grammaticavoorbeeld, dus die blijft bruikbaar zonder werkende spraak-engine
